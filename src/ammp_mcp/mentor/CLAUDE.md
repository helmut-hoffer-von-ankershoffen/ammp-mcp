...
- `_cli.py` — `mentor_app` Typer subgroup. `ammp mentor list` inspects the registry; `--json` emits the same envelope the MCP `ListMentors` extension returns (CLI parity for bash-plus-CLI agents). `ammp mentor ask` / `ammp mentor escalate` give the shell CLI parity with the AMMP `AskMentor` / `EscalateToHuman` ops by invoking the same in-process handlers `server.py` exposes over MCP. Local CLI bypasses Bearer auth (operator already has filesystem access).
